...
if __name__ == "__main__":
    ...
    benchmark = database_loader.load_questions("../assets/benchmarks/openbookqa/test.jsonl")

    language_models = [CohereAPI(stop_token="\n"), BLOOM176bAPI(stop_token="\n")]
    information_retrievers = [AnnoyRetriever("openbookqa",
    ...
    for language_model in language_models:
        for information_retriever in information_retrievers:
            for k in range(1, 4):
                ...
                save_results(f"./result/openbookqa/k_documents/{language_model}/{information_retriever}/{k}_documents.json", results)
        ...
        save_results(f"./result/openbookqa/k_documents/{language_model}/0_documents.json", results)
